    ...
    if (wsAgentThoughts && wsAgentThoughts.length > 0) {
      ...
      try {
        const convertedThoughts: AgentMessage[] = wsAgentThoughts.map(thought => {
          ...
          let timestamp;
          try {
            const tmpDate = thought.timestamp;
            timestamp = (typeof tmpDate === 'object' && tmpDate !== null && 'getTime' in tmpDate) 
              ? tmpDate.getTime() 
              : new Date(String(tmpDate)).getTime();
...
// 外部から渡されたthoughtsを内部形式に変換
  ...
  if (height) {
    return (
      <div className="w-full h-full flex flex-col agent-thoughts-container">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              <TabsContent value={activeTab} className="m-0 flex-1 flex flex-col overflow-hidden">
                <div className="flex-1 overflow-auto">
                  <ScrollArea className="agent-thoughts-scroll w-full px-4 py-4" style={{ height: '100%' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            <TabsContent value={activeTab} className="m-0 flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-auto">
                <ScrollArea className="agent-thoughts-scroll w-full px-6 py-4" style={{ height: 'calc(100% - 10px)' }}>
                  {filteredThoughts.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-40">
