    ...
    setTimeout(() => {
      sendAgentThoughts(
        ...
        roleModelId,
        {
          ...
          reasoning: `業界分析の視点からWebマーケティングコンサルティング市場を評価するにあたり、以下の点が重要です：

1. コンサルティング企業のデジタル変革ニーズが急速に拡大している
2. 人材不足と作業の効率化がクライアント企業の主要課題となっている
3. 競合状況は多数の専門特化プレイヤーと大手総合コンサルの二極化が進んでいる
4. 市場成長率は年率15-20%程度と予測されるが、専門性によって大きく異なる

これらの要素を総合すると、AIエージェントによる業務効率化とマーケティング知見の体系化が競争優位性を生み出す可能性が高いと考えられます。`,
          steps: [
            "主要なWebマーケティングコンサルティング業界セグメントと市場規模を調査",
            "主要プレイヤーとサービス提供の分析（大手およびニッチプレイヤー）",
            "クライアントニーズと課題の明確化（詳細なインタビューデータ分析）",
            "現在のAIエージェント活用状況調査（採用率、成功・失敗事例）",
            "競合分析とポジショニングマップの作成"
          ],
          details: {
            industrySize: "グローバルWebマーケティングコンサル市場: 約1,200億ドル（2023年）",
            growthRate: "年間成長率: 15-20%（2024-2028予測）",
            keyPlayers: ["Accenture Interactive", "Deloitte Digital", "PwC Digital", "専門特化企業（Jellyfish, Wpromote）"],
            aiAdoptionRate: "Webマーケティング業界でのAI活用率: 約30%（2023年データ）",
            challengeAreas: ["ターゲティング精度", "コンテンツ作成効率", "データ分析・解釈", "施策効果測定"]
          }
        }
      );
